...
function InitControl(obj,h)   
    ...
    obj.border = 1 
    if h.layout then
        if h.layout.proportion then obj.proportion = h.layout.proportion end
...
function EzBoxSizer(orient)
    ...
    sizer.Add = function(child) 
        local proportion = 0
        local border = 0
        local flags = wx.wxALIGN_CENTER + wx.wxALL
        if child.expand then flags = flags + wx.wxEXPAND end
        if child.border then border = child.border end 
        if child.proportion then proportion = child.proportion end
